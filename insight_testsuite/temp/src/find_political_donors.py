...
def process_data(data,Judge):
    drop_list=[] #the line we need to drop for
    new_zip=[]
    new_AMT=[]
    #make a dataframe
...
def round_float(lis):
    ...
    return round_lis
#apply()
def question1(output_path_zip,df):
...
def question2(output_path_date,df):
...
if __name__ == "__main__":
    ...
    data=get_data(local_address+'/input/itcont.txt')
    df=process_data(data,'zip')
    a=question1(local_address+'/output/medianvals_by_zip.txt',df)
    df=process_data(data,'date')
    b=question2(local_address+'/output/medianvals_by_date.txt',df)
